...
ifneq (,$(strip $(NATIVE_TARGET)))
  ...
  NATIVE_EXE:=out/penance.$(NATIVE_TARGET)
  all:$(NATIVE_EXE)
...
ifeq ($(NATIVE_TARGET),macos)
  BUNDLE:=out/penance.app
  BUNDLE_EXE:=$(BUNDLE)/Contents/MacOS/penance
  ...
  $(BUNDLE_PLIST):$(EGG_SDK)/src/opt/macos/Info.plist $(EGG_SDK)/etc/tool/plist.sh; \
    $(PRECMD) $(EGG_SDK)/etc/tool/plist.sh $(EGG_SDK)/src/opt/macos/Info.plist src/data/metadata penance com.aksommerville.penance > $@
